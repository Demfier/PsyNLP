    ...
    def convert2cytoscapejson(self):
        ...
        for concept in concepts.values():
            ...
            if len(intent) > 1 and len(extent) > 1:
                ...
                e_node["data"]["canonicalName"] = "{} objects".format(len(concept["extent"]))
                e_node["data"]["Type"] = "Extent"
                e_node["data"]["NodeType"] = "ExtentNode"
                e_node["data"]["name"] = "{} objects".format(len(concept["extent"]))
                e_node["data"]["shared_name"] = "{} objects".format(len(concept["extent"]))
